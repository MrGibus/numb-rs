...
/// this import allows for indexing the matrix
/// for example a[[0,1]]
use std::ops::{Index, IndexMut, Deref, DerefMut, Mul, MulAssign, AddAssign};
use std::fmt::Display;
...
impl<'a, T> IntoIterator for &'a Matrix<T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<'a, T> Iterator for MatrixIterator<'a, T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<'a,'b, T> IntoIterator for &'b MatrixT<'a, T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<'a, 'b, T> Iterator for MatrixTIterator<'a, 'b, T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<'a, T> IntoIterator for &'a MatrixS<T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<'a, T> Iterator for MatrixSIterator<'a, T> where T: Copy{
    type Item = (T, [usize;2]);
...
impl<T> Mul<Matrix<T>> for Matrix<T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
...
impl<T> Mul<MatrixS<T>> for Matrix<T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
    type Output = Result<Self, MatrixError>;
...
impl<T> Mul<MatrixS<T>> for MatrixS<T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
...
impl<T> Mul<Matrix<T>> for MatrixS<T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
    type Output = Result<Matrix<T>, MatrixError>;
...
impl<T> Mul<MatrixT<'_, T>> for Matrix<T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
    type Output = Result<Self, MatrixError>;
...
impl<T> Mul<Matrix<T>> for MatrixT<'_, T> where T: Mul<Output = T> + Copy + AddAssign + Zero{
    type Output = Result<Matrix<T>, MatrixError>;
...
impl<T> Index<[usize; 2]> for Matrix<T>{
    ...
    fn index(&self, idx: [usize; 2]) -> &T { &self.data[idx[1] + idx[0] * self.n] }
}
...
impl<T> Index<[usize;2]> for MatrixT<'_, T>{
    ...
    fn index(&self, idx: [usize; 2]) -> &T { &self.data[idx[0] + idx[1] * self.m] }
}

impl<T> Index<[usize; 2]> for MatrixS<T> where T: Copy{
    type Output = T;
    ...
    ($int:ty) => {
        impl Display for Matrix<$int> {
            fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
                let s = self.into_iter().fold(String::new(), |mut acc, (x, [_i, j])| {
...
impl ApproxEq<f64> for f64{
    ...
    left: `{:?}`
    right: `{:?}`
    delta: `{:?}`"#, tolerance, &*self, &*other, delta)
        }
    }
}
...
impl ApproxEq<Matrix<f64>> for Matrix<f64> {
    ...
    left  m x n: `{:?}`x`{:?}`
    right m x n: `{:?}`x`{:?}`"#, self.m, other.m, self.n, other.n)
        }
...
mod tests {
    ...
    fn matrix_print() {
        ...
        assert_eq!(format!("{}", a), "  1.00  2.00  3.00\n  4.00  5.00  6.00\n  7.00  8.00  9.00\n"
            .to_string());

        assert_eq!(format!("{}", a.t()), "  1.00  4.00  7.00\n  2.00  5.00  8.00\n  3.00  6.00  9.00\n"
            .to_string());
        ...
        assert_eq!( format!("{}", c), "  1.00  2.00  4.00\n  2.00  3.00  5.00\n  4.00  5.00  6.00\n"
            .to_string());
